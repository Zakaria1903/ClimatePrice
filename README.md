...
**https://climateprice-94708382002.europe-west4.run.app**

> The current deployment uses the synthetic twin dataset. It will automatically display the real Paris IRIS boundaries once `joined.geojson` is generated by the data pipeline.
...
```bash
docker build -t climateprice .
```

Run locally:

```bash
docker run -p 8080:8080 climateprice
```

The Docker image generates the synthetic dataset during the build and starts the Streamlit application automatically.
